...
describe('planner (chase behavior for bone-knight)', () => {
  ...
  it('makes sideways progress when primary axis is blocked by another actor', () => {
    ...
    const action = decide(w, enemyId)
    // The enemy cannot go west (blocker) — must either move north/south (sideways) or TurnAdvance.
    ...
    // So the action should be a MoveActor to a passable neighbor (if any) or AttackActor
    // if somehow adjacent — but it isn't, so MoveActor or TurnAdvance (if all 4 blocked by walls).
    if (action.type === 'MoveActor') {
      const p = action.to
